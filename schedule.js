// schedule.js (JavaScript Murni dengan DOM Manipulation)
...
const renderLessonItem = (lesson, idx) => {
    ...
    const breakClasses = isBreak ? 'border-yellow-300 dark:border-yellow-600' : 'border-slate-200 dark:border-slate-700';
    const markerClasses = isBreak ? 'bg-yellow-400' : 'bg-blue-500';
    const timeColor = isBreak ? 'text-yellow-600 dark:text-yellow-500' : 'text-blue-600 dark:text-blue-400';
    const subjectColor = isBreak ? 'text-slate-500 italic' : 'text-slate-900 dark:text-white';

    return `
        <li key="${idx}" class="relative pl-6 border-l-2 ${breakClasses} pb-3 last:pb-0 last:border-0">
            ...
            <div class="flex flex-col">
                ...
                ${lesson.code && !isBreak ? `
                    <span class="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1 mt-1">
